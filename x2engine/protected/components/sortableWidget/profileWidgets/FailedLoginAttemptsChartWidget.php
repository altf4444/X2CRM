 ...
 */
class FailedLoginAttemptsChartWidget extends ChartWidget {
	 ...
	 */
	protected function getInitialChartData ($dataStartDate) {
		...
		$dataStartDate, time () + self::SECPERDAY);
            ...
            $failedEvents = self::getChartData ($startDate, $endDate);
            return $failedEvents;
	}
	 ...
	 */
	protected function getFirstActionDate () {

            $command = Yii::app()->db->createCommand()
            ->select('min(lastAttempt)')
            ->from('x2_failed_logins');
